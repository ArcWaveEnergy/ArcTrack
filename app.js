...
function setButtons(running) { $('#startBtn').disabled = running; $('#stopBtn').disabled = !running; }

function saveLastJob(jobId) { localStorage.setItem('arctrack:lastJob', String(jobId)); }
function getLastJob() { return Number(localStorage.getItem('arctrack:lastJob') || 0); }

async function loadUser() {
  const me = await fetchJSON('/api/me'); const el = document.getElementById('userEmail'); if (el) el.value = me.email || '';
}
...
async function loadJobs(selectId) {
  ...
  const sel = $('#activeJob'); const list = $('#jobs'); sel.innerHTML = ''; list.innerHTML = '';
  for (const j of jobs) {
  ...
  list.querySelectorAll('button.send').forEach(b => {
    b.onclick = async () => {
      ...
      if (res.ok) alert('Report generated' + (res.email?.sent === false ? ` (email error: ${res.email.error})` : ''));
